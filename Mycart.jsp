...
<div id="myCartMain">
	<div id="shopTop">
		<a href="MyCart.do?headerr=six"><p>CART</p></a>
	</div>
	<div id="mycart-logo">
	...
	<form action="delete.do" method="get">
		<table width="800" height="30" id="cartable">
			...
			<input type="hidden" id="code${cnt}" value="${mcto.item_code }">
				<tr id="cartr">
					...
					<c:out value="₩${price}"></c:out><input type="hidden" id="price${cnt}" value="${mcto.item_price }"></td>
					<td><input class="plma" type="button" value="-" onclick="plus(${cnt})" ></td>
					<td><input type="text"  value="${mcto.cnt }" readonly="readonly" id="pronum${cnt}" class="pronum" ></td>
					<td><input class="plma" type="button" value="+" onclick="minus(${cnt})" ></td>
					<td id="sum${cnt }"><fmt:formatNumber var="prosum" value="${mcto.item_price * mcto.cnt}" pattern="#,###" />
					<c:out value="₩${prosum}"></c:out></td>
				</tr>
		...
		<br>
				<table id="buttontable">
					...
					<td><input type="submit" value="전체삭제" formaction="alldelete.do"></td>
					<td><input type="submit" value="결제로 진행" formaction="itembuy.do"></td>
				</table>
			</div>
			<br><br>
			<div id="sumdiv">
				...
				<table>
				    <tr>
				 		<td colspan="2"><h2>장바구니 총계</h2></td>
						
					</tr>
					<tr>
					<input id="realsum1" type="hidden" readonly="readonly" value="${sum }">
						<td>소계　</td>
						<td id="gun1"><fmt:formatNumber var="sum2" value="${sum}" pattern="#,###" />
					...
					<tr>
					<input id="realsum2" type="hidden" readonly="readonly" value="${sum }">
						<td>총계　</td>
...
function minus(num){
    ...
    function ckbox(num){
    	...
    	if(ck.checked == true){
    		...
    		gun.innerHTML = "₩"+  parseInt((realsum1.value)).toLocaleString();
    		gun1.innerHTML = "₩"+ parseInt((realsum2.value)).toLocaleString();
    	}else{
    		realsum1.value = parseInt(realsum1.value) - parseInt(prosum.value);
    		realsum2.value = parseInt(realsum2.value) - parseInt(prosum.value);
    		gun.innerHTML = "₩"+  parseInt((realsum1.value)).toLocaleString();
    ...
    function  allckbox(){
    	...
    	if(all.checked == true){
    		...
    		realsum1.value = ${sum};
    		realsum2.value = ${sum};
    		gun.innerHTML = "₩"+  parseInt(${sum}).toLocaleString();
    		gun1.innerHTML = "₩"+ parseInt(${sum}).toLocaleString();
    	}else{
    		for(var i = 1; i<${cnt} ; i++ ){
    			var ck=  document.getElementById("ck"+i);
    			ck.checked = false;
